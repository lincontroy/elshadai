...
class userController extends Controller
{
  ...
  public function verify_u2s(Request $req)
  {
    ...
    try
    {
      if(Google2FA::verifyGoogle2FA(Session::get('temp_2fa_key'), $req['otp']))
      {
        ...
        return redirect()->route('login')->with([
          'toast_msg' => 'User credentials are not correct!',
          'toast_type' => 'err'
        ]);
      }
      else
  ...
  {  
      ...
      {         
        ...
        {         
          ...
          Session::put('status', "Details saved Successfully");
          Session::put('msgType', "suc");
  ...
  {        
    ...
    public function wd_invests(Request $req){
  ...
  {        
      ...
      if(intval($req->input('amt')) > intval($user->wallet) || intval($req->input('amt')) == 0 )
      {
        ...
        return back()->with('error','Invalid withdrawal amount. Amount must be greater than zero(0) and not more than wallet balance');
      }  
           ...
           if(intval($req->input('amt')) < env('MIN_WD'))
      {
        ...
        return back()->with('error','Invalid withdrawal amount. Amount must be greater than ' .env('MIN_WD'));
      }
      ...
      if(intval($req->input('amt')) > env('WD_LIMIT'))
      {
        ...
        return back()->with('error',env('WD_LIMIT').' Withdrawal limit exceeded!');
      }
      ...
      {         
        ...
        {  
          ...
          Session::put('msgType', "suc");
           return back()->with('success','Wallet Withdrawal Successful! Note: Withdrawal attracts '.$wd_fee.'% processing fee');
        }
        catch(\Exception $e)
        {
  ...
  {        
      ...
      try
      {
          $rec = User::where('phone', trim($req->input('phone')))->get();
          if(count($rec) < 1)
          {
              Session::put('err_send', "Phone record not found!");
              Session::put('status', 'Phone record not found!');      
              Session::put('msgType', "err");
              return back();
          }
          ...
          $rc->to_usr = trim($req->input('phone'));
          $rc->amt = intval($req->input('s_amt'));
          $rc->save();
          
          $act = new activities;
          $act->action = "User send fund of ".$user->currency.intval($req->input('s_amt'))." to ".trim($req->input('phone'));
          $act->user_id = $user->id;
          $act->save();
  ...
  public function user_send_funds(Request $req)
  {        
      $user = Auth::User();
