  ...
  {        
    ...
    {       
      ...
      try
      {
        ...
        $level_one_upline=User::where('id',$id)
        
        ->get('referal');
        
        if((count($level_one_upline))>0){
            
            //get the upline username of this user
            foreach($level_one_upline as $level_one){
                
                $upline_user_name=$level_one['referal'];
                
                //get the referal balance of this upline and add the balance for level 1
                
                
                $upline_one_balances=User::where('username',$upline_user_name)->get('ref_bal');
                
                foreach($upline_one_balances as $upline_one_balance){
                    $old_balance=$upline_one_balance['ref_bal'];
                    $new_balance=$old_balance + env('level_one');
                    
                    //now update this balance 
                    $updater=User::where('username',$upline_user_name)->update(['ref_bal'=>$new_balance]);
                    
                    
                //   echo $upline_user_name;
                    
                    
                    
                }
                //check if this level one has an upline and award this upine level two bonus
                $level_two_upline_usernames=User::where('username',$upline_user_name)
                ->where('status',1)
                ->get('referal');
                
                foreach($level_two_upline_usernames as $level_two_upline_username){
                $upline_two_username=$level_two_upline_username['referal'];
                
                $upline_two_balances=User::where('username',$upline_two_username)->get('ref_bal');
                
                  foreach($upline_two_balances as $upline_two_balance){
                    $old_balance=$upline_two_balance['ref_bal'];
                    $new_balance=$old_balance + env('level_two');
                    
                    //now update this balance 
                    $updater=User::where('username',$upline_two_username)->update(['ref_bal'=>$new_balance]);
                    
                  
                    // echo $upline_two_username;
            ...
            // Session::put('status', "Error, no upline");
            // Session::put('msgType', "err");
        }
